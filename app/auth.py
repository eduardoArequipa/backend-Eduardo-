...
# Definida aquí, en auth.py, para evitar importaciones circulares.
RECOVERY_CODE_EXPIRE_MINUTES = int(os.getenv("RECOVERY_CODE_EXPIRE_MINUTES", 15))# Configuración de seguridad
...
ACCESS_TOKEN_EXPIRE_MINUTES = int(os.getenv("ACCESS_TOKEN_EXPIRE_MINUTES", 30)) # Default 30 minutos
...
async def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    try:
        ...
        username: str = payload.get("sub") # 'sub' es el sujeto del token, usaremos el nombre de usuario

        if username is None:
            raise credentials_exception

        # Puedes añadir validación adicional aquí si incluyes más datos en el token, e.g., roles
    ...
    user = db.query(Usuario).filter(Usuario.nombre_usuario == token_data.username).first()
...
# Dependencia para obtener un usuario activo (no inactivo/bloqueado)
def get_current_active_user(current_user: Usuario = Depends(get_current_user)):
    ...
    if current_user.estado == EstadoEnum.bloqueado:
        if current_user.bloqueado_hasta and current_user.bloqueado_hasta > datetime.now(timezone.utc):
            ...
            # significa que debería ser activado por un login exitoso.
            # Aquí, para fines de acceso a rutas protegidas, lo consideramos como no activo
            # hasta que un login exitoso lo restablezca a 'activo'.
            # Esto evita que un usuario bloqueado acceda a rutas si no ha completado el proceso de login exitoso.
...
def get_current_active_user_with_role(required_roles: List[str]):
    ...
    """
    def _get_current_active_user_with_role(
        current_user: Usuario = Depends(get_current_active_user),
        db: Session = Depends(get_db)
    ):
        # Cargar los roles del usuario si no están ya cargados
        db.refresh(current_user, attribute_names=['roles']) # Asegurar que la relación roles está cargada

        user_roles_names = [rol.nombre_rol for rol in current_user.roles]
        ...
        return current_user
    return _get_current_active_user_with_role
