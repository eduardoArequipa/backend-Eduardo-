...
class Usuario(Base):
    ...
    # La relación 1:1 con persona. 'UNIQUE=True' en el FK enforcement la unicidad en este lado.
    ...
    expiracion_codigo_recuperacion = Column(DateTime(timezone=True), nullable=True) # <-- ¡CAMBIO AQUÍ!    # Relación 1:1 con Persona
    persona = relationship("Persona", back_populates="usuario", uselist=False) # uselist=False para 1:1

    # Relación N:N con Rol a través de la tabla usuario_roles
    # El nombre de la tabla secundaria debe coincidir con tu definición SQL
    ...
    compras_creadas = relationship("Compra", foreign_keys="[Compra.creado_por]", back_populates="creador")
    ...
    compras_modificadas = relationship("Compra", foreign_keys="[Compra.modificado_por]", back_populates="modificador")
